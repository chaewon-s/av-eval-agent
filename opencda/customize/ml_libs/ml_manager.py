...
class MLManager(object):
    ...
    def __init__(self):
        if torch is None:
            print('[ML] torch is not installed; perception will use the '
                  'semantic-lidar fallback for datadump.')
        ...
        try:
            ...
            self.object_detector = torch.hub.load(
                'ultralytics/yolov5', 'yolov5m', trust_repo=True)
        except Exception as exc:
            print('[ML] failed to load yolov5m (%s); perception will use '
                  'the semantic-lidar fallback for datadump.' % exc)
            self.object_detector = None
