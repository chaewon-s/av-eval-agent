...
def _set_color_if_supported(blueprint, color):
    """Set blueprint color only when the vehicle supports it."""
    if blueprint.has_attribute('color'):
        blueprint.set_attribute('color', color)


def _find_first_matching_blueprint(blueprint_library, model_names):
    for model_name in model_names:
        matches = blueprint_library.filter(model_name)
        if matches:
            return matches[0], model_name
    return None, None


def _resolve_vehicle_blueprint(blueprint_library, model_name, fallback_model=None):
    """
    Resolve a vehicle blueprint across CARLA versions with a small alias table.
    """
    candidates = [model_name]
    candidates.extend(BLUEPRINT_COMPATIBILITY_MAP.get(model_name, []))
    if fallback_model:
        candidates.append(fallback_model)
        candidates.extend(BLUEPRINT_COMPATIBILITY_MAP.get(fallback_model, []))
    ...
    blueprint, resolved_name = _find_first_matching_blueprint(
        blueprint_library,
        ordered_candidates)

    if blueprint is None:
        raise ValueError(
            'No compatible blueprint found for %s' % model_name)

    if resolved_name != model_name:
        print("Blueprint '%s' not found. Using '%s' instead." %
              (model_name, blueprint.id))

    return blueprint


def _resolve_blueprint_list(blueprint_library, model_names):
    """
    Resolve a list of blueprint ids while skipping unavailable ones.
    """
    resolved = []
    seen = set()
    ...
    for model_name in model_names:
        ...
        blueprint, _ = _find_first_matching_blueprint(blueprint_library,
                                                      candidates)
        if blueprint and blueprint.id not in seen:
            resolved.append(blueprint)
...
def car_blueprint_filter(blueprint_library, carla_version='0.9.11'):
    ...
    blueprints = _resolve_blueprint_list(blueprint_library, blueprint_names)
    return blueprints
...
def multi_class_vehicle_blueprint_filter(label, blueprint_library, bp_meta):
    ...
    """
    blueprints = _resolve_blueprint_list(
        blueprint_library,
        [k for k, v in bp_meta.items() if v["class"] == label])
    return blueprints
...
class ScenarioManager:
    ...
    def __init__(self, scenario_params,
        ...
        if xodr_path:
            ...
            for _ in range(5):
                try:
                    ...
                    try:
                        ...
                        if current_map_name.endswith(town):
            ...
            if not self.world:
                ...
                if any(map_name.endswith('/%s' % town) or
                       map_name.endswith(town) for map_name in available_maps):
                    print(
                        f"{bcolors.FAIL} Failed to load %s after retries: %s"
                        f"{bcolors.ENDC}" % (town, last_error))
                else:
                    print(
        ...
        if simulation_config['sync_mode']:
            ...
            # new_settings.fixed_delta_seconds = (1.0 / simulation_config["fps"]) if simulation_config["fps"] > 0.0 else 0.0
        else:
            sys.exit(
        ...
        weather = self.set_weather(
            simulation_config['weather']
            if 'weather' in simulation_config else DEFAULT_WEATHER)
    ...
    def create_vehicle_manager(self, application,
        ...
        for i, cav_config in enumerate(
            ...
            if 'vehicle_type' in cav_config:
                model_name = cav_config['vehicle_type']
            else:
                model_name = default_model  # 기본 모델 사용

            cav_vehicle_bp = _resolve_vehicle_blueprint(
                self.world.get_blueprint_library(),
                model_name,
                default_model)

            vehicle_color = cav_config.get('color', '0, 0, 255')
            _set_color_if_supported(cav_vehicle_bp, vehicle_color)
    ...
    def create_walker_manager(self, 
                               map_helper=None,
                               data_dump=False):
        """
        Create a list of single CAVs.
        ...
        for i, cav_config in enumerate(
                self.scenario_params['scenario']['walker_list']):
            ...
            walker_controller = self.world.get_blueprint_library().find('controller.ai.walker')
            # walker_bp.set_attribute('color', '0, 0, 255')
            # walker = SpawnActor(walker_controller, carla.Transform(), walker_bp)
            walker = self.world.spawn_actor(walker_bp, spawn_transform)
            walker = self.world.spawn_actor(walker_controller, carla.Transform(), walker)
            ...
            #     current_time=self.scenario_params['current_time'],
            #     data_dumping=data_dump)
            self.destination = carla.Location(x=cav_config['destination'][0],
                                         y=cav_config['destination'][1],
                                         z=cav_config['destination'][2])

            walker.start()
            walker.go_to_location(self.destination)
    ...
    def create_platoon_manager(self, map_helper=None, data_dump=False):
        ...
        for i, platoon in enumerate(
            ...
            for j, cav in enumerate(platoon['members']):
                ...
                _set_color_if_supported(cav_vehicle_bp, '0, 0, 255')
    ...
    def spawn_vehicles_by_list(self, tm, traffic_config, bg_list):
        ...
        default_model = 'vehicle.lincoln.mkz2017' \
            if self.carla_version == '0.9.11' else 'vehicle.lincoln.mkz_2017'
        ego_vehicle_bp = _resolve_vehicle_blueprint(blueprint_library,
                                                    default_model)
        ...
        for i, vehicle_config in enumerate(traffic_config['vehicle_list']):
            ...
            if 'vehicle_type' in vehicle_config:
                ego_vehicle_bp = _resolve_vehicle_blueprint(
                    blueprint_library,
                    vehicle_config['vehicle_type'],
                    default_model)
            elif not traffic_config['random']:
                _set_color_if_supported(ego_vehicle_bp, color)
            ...
            if 'color' in vehicle_config:
                _set_color_if_supported(ego_vehicle_bp,
                                        vehicle_config['color'])
    ...
    def create_pedestrian_manager(self, pedestrian_count=20, pedestrian_speed=1.0):
        """
        ...
        pedestrian_count : int
            The number of pedestrians to spawn.

        pedestrian_speed : float
            The speed at which the pedestrians should walk.

        Returns
        -------
        pedestrian_list : list
            A list containing all the pedestrian actors.
        """
        ...
        for _ in range(pedestrian_count):
            # Randomly select a blueprint for the pedestrian
            pedestrian_bp = random.choice(pedestrian_blueprints)
            
            # Randomly select a spawn location on the map
            spawn_point = self.world.get_random_location_from_navigation()
            if spawn_point:
                pedestrian = self.world.try_spawn_actor(pedestrian_bp, carla.Transform(spawn_point))
                if pedestrian:
                    pedestrian_list.append(pedestrian)

                    # Create a controller for the pedestrian
                    walker_controller_bp = blueprint_library.find('controller.ai.walker')
                    walker_controller = self.world.spawn_actor(walker_controller_bp, carla.Transform(), pedestrian)
                    
                    # Set pedestrian control parameters
                    walker_controller.start()
                    walker_controller.go_to_location(self.world.get_random_location_from_navigation())
                    walker_controller.set_max_speed(pedestrian_speed)  # in meters per second

                    pedestrian_list.append(walker_controller)
        
        print(f'{len(pedestrian_list) // 2} pedestrians have been created.')
