...
class CameraSensor:
    ...
    def __init__(self, vehicle, world, relative_position, global_position):
        ...
        # blueprint.set_attribute('shutter_speed', '10')
...
class SemanticLidarSensor:
    ...
    def __init__(self, vehicle, world, config_yaml, global_position):
        ...
        # spawn sensor
        if global_position is None:
            spawn_point = carla.Transform(carla.Location(x=-0.5, z=1.9)) # z = 1.9
        else:
            spawn_point = carla.Transform(carla.Location(x=global_position[0],
                                                         y=global_position[1],
                                                         z=global_position[2]))
...
class PerceptionManager:
    ...
    def __init__(self, vehicle, config_yaml, cav_world,
        ...
        try:
            self.connected = config_yaml['connected']
        except:
        ...
        if self.activate and data_dump:
            print('[PERCEPTION-DUMP] perception and datadump are both '
                  'enabled; prediction objects and CARLA ground truth will '
                  'be dumped together.')

        if self.activate and not ml_manager:
            if data_dump:
                print('[PERCEPTION-DUMP] apply_ml was not provided; using '
                      'semantic-lidar fallback detections for datadump.')
            else:
                sys.exit(
    ...
    def detect(self, ego_pos):
        ...
        if self.connected:
            objects = self.connect_mode(objects)
    ...
    def activate_mode(self, objects):
        ...
        """
        object_detector = getattr(self.ml_manager, 'object_detector', None)
        if object_detector is None:
            objects = self.deactivate_mode(objects)
            objects['perception_mode'] = 'semantic_lidar_fallback'
            self._assign_prediction_tracks(objects)
            self._attach_gt_vehicle_snapshot(objects)
        ...
        # yolo detection
        yolo_detection = object_detector(rgb_images)
        ...
        objects['perception_mode'] = 'yolov5_lidar_fusion'
        objects.setdefault('vehicles_pcd', {})
        self._assign_prediction_tracks(objects)
        self._attach_gt_vehicle_snapshot(objects)
        self.objects = objects
    ...
    def _assign_prediction_tracks(self, objects):
        """
        Attach stable prediction IDs to perception detections for MOT KPIs.
        YOLO/LiDAR fusion detections do not have CARLA IDs, so a lightweight
        nearest-neighbor tracker is used only for logging and evaluation.
        """
        if not hasattr(self, '_prediction_tracks'):
            self._prediction_tracks = {}
            self._next_prediction_track_id = 1

        vehicle_detections = objects.get('vehicles', [])
        carla_vehicles = [
            v for v in self.carla_world.get_actors().filter("*vehicle*")
            if v.id != self.id]
        updated_tracks = {}

        for detection in vehicle_detections:
            detection_location = detection.get_location()
            carla_id = getattr(detection, 'carla_id', -1)

            if carla_id != -1:
                track_id = carla_id
                matched_gt_id = carla_id
                match_distance = 0.0
            else:
                track_id = None
                best_track_distance = 999999.0
                for candidate_id, candidate_location in \
                        self._prediction_tracks.items():
                    distance = detection_location.distance(candidate_location)
                    if distance < best_track_distance:
                        best_track_distance = distance
                        track_id = candidate_id

                if track_id is None or best_track_distance > 4.0:
                    track_id = self._next_prediction_track_id
                    self._next_prediction_track_id += 1

                matched_gt_id = -1
                match_distance = 999999.0
                for vehicle in carla_vehicles:
                    distance = detection_location.distance(
                        vehicle.get_location())
                    if distance < match_distance:
                        match_distance = distance
                        matched_gt_id = vehicle.id

                if match_distance > 5.0:
                    matched_gt_id = -1

            detection.prediction_id = int(track_id)
            detection.matched_gt_id = int(matched_gt_id)
            detection.match_distance = float(match_distance)
            updated_tracks[int(track_id)] = carla.Location(
                x=detection_location.x,
                y=detection_location.y,
                z=detection_location.z)

        self._prediction_tracks = updated_tracks

    def _attach_gt_vehicle_snapshot(self, objects):
        """
        Add all non-ego CARLA vehicles as ground-truth objects for evaluating
        active perception predictions in the same datadump YAML.
        """
        vehicle_list = [
            v for v in self.carla_world.get_actors().filter("*vehicle*")
            if v.id != self.id]
        ...
        objects['total_vehicles'] = total_vehicle_list
        objects.setdefault('total_vehicles_pcd', {})
    ...
    def deactivate_mode(self, objects):
        ...
        # todo: hard coded
        thresh = 50 if not self.data_dump else 50

        filtered_vehicle_list = [v for v in vehicle_list if self.dist(v) < thresh and
                        v.id != self.id] ## vehicles in sight distance = 50
        # use semantic lidar to filter out vehicles out of the range

        ## total_vehicle_list : to export pcd_list, not for control, only for record.
        ## rest_vehicle_list: exising actors
        pcd_list = []
        rest_vehicle_list = []
        total_pcd_list = []
        total_vehicle_list = []
        if self.data_dump:
            if (len(filtered_vehicle_list) != 0) or filtered_vehicle_list is not None:
                filtered_vehicle_list, pcd_list = self.filter_vehicle_out_sensor(filtered_vehicle_list)
                rest_vehicle = list(set(vehicle_list)-set(filtered_vehicle_list))
                if len(rest_vehicle) != 0:
                    rest_vehicle_list = [v for v in rest_vehicle if v.id != self.id]
                    _, total_pcd_list = self.filter_vehicle_out_sensor(rest_vehicle_list)
                else:
                    rest_vehicle_list = []  ## consider the vehicles not observed by ego
                    total_pcd_list = []
            else: 
                pcd_list = []
                rest_vehicle_list = []
        ...
        if self.lidar:
            vehicle_list = [
                ObstacleVehicle(
                    ...
                    v,
                    self.lidar.sensor,
                    self.cav_world.sumo2carla_ids) for v in filtered_vehicle_list]
            
            ## TOTAL VEHICLE LIST including invisible vehicles
            if len(rest_vehicle_list) != 0:
                total_vehicle_list = [
                    ObstacleVehicle(
                        None,
                        None,
                        v,
                        None,
                        self.cav_world.sumo2carla_ids) for v in rest_vehicle_list]
        else:
            vehicle_list = [
                ObstacleVehicle(
                    None,
        ...
        objects.update({'vehicles_pcd': pcd_list})
        
        if len(rest_vehicle_list) != 0:
            objects.update({'total_vehicles': total_vehicle_list})
            objects.update({'total_vehicles_pcd': total_pcd_list})
    ...
    def connect_mode(self, objects):
        """
        Object detection using connected server information directly by sharing others' positions.
        ...
        total_vehicle_list = [v for v in vehicle_list if v.id != self.id]
        _, total_pcd_list = self.filter_vehicle_out_sensor(total_vehicle_list)

        vehicle_list_by_server = [v for v in vehicle_list if self.dist(v) < thresh and
                        v.id != self.id]
        
        vehicle_list = [v for v in vehicle_list if self.dist(v) < 50 and
                        v.id != self.id] # in sightdistance(50)
        # use semantic lidar to filter out vehicles out of the range

        if self.data_dump:
            filtered_vehicle_list, pcd_list = self.filter_vehicle_out_sensor(vehicle_list)
        ## consider the vehicles not observed by ego.

        vehicle_list_by_server.extend(filtered_vehicle_list)
        vehicle_list = list(set(vehicle_list_by_server)) ## this is vehicle by V2X and LiDAR
        ...
        if self.lidar:
            ...
            total_vehicle_list = [
                ObstacleVehicle(
                    ...
                    v,
                    self.lidar.sensor,
                    self.cav_world.sumo2carla_ids) for v in total_vehicle_list]
        else:
            vehicle_list = [
                ObstacleVehicle(
                    None,
    ...
    def filter_vehicle_out_sensor(self, vehicle_list):
        ...
        points = self.semantic_lidar.points
        try:
            min_length = min(len(semantic_idx), len(semantic_tag))
        except: 
            return [], []
        semantic_idx = semantic_idx[:min_length]
        semantic_tag = semantic_tag[:min_length]
        ...
        for veh in vehicle_list: 
            if veh.id in vehicle_unique_id:
                ...
                pcd_list[veh.id]= points[semantic_idx == veh.id]
        return new_vehicle_list, pcd_list
    ...
    def speed_location_retrieve(self):
        """
        We don't implement any obstacle speed calculation algorithm.
        The speed will be retrieved from the server directly.
        ...
        for v in vehicle_list:
            ...
            if speed > 0:
                continue
            obstacle_loc = v.get_location()
            if abs(loc.x - obstacle_loc.x) <= 3.0 and \
                    abs(loc.y - obstacle_loc.y) <= 3.0:
                v.set_velocity(v.get_velocity())
                v.set_carla_id(v.id)
            veh_dict[v.id] = {"location": loc, "speed": speed}            
        return veh_dict
