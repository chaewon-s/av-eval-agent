...
SCENARIO1_IGNORED_VEHICLE_TYPES = {
    "vehicle.mitsubishi.fusorosa",
    "vehicle.mercedes.sprinter",
}


class BehaviorAgent1(object):
    """
    ...
    def __init__(self, vehicle, carla_map, config_yaml):
        ...
        self._collision_check = CollisionChecker(
            time_ahead=config_yaml['collision_time_ahead'], circle_radius = 3)
        self._potential_collision_check = CollisionChecker(time_ahead=config_yaml['collision_time_ahead'],
                                                           circle_radius=5)
    ...
    def traffic_light_manager(self, waypoint):
        ...
        #     self.stop_sign_wait_count = 0
        # self.light_state = "Green" # 비신호 교차로

        if 60 <= self.stop_sign_wait_count < 120:
            self.stop_sign_wait_count += 1
        elif self.stop_sign_wait_count >= 120:
            self.stop_sign_wait_count = 0
        self.light_state = "Green" # 비신호 교차로
    ...
    def collision_manager(self, rx, ry, ryaw, waypoint, adjacent_check=False):
        ...
        for vehicle in self.obstacle_vehicles:
            ## scenario 예외
            if getattr(vehicle, 'type_id', '') in SCENARIO1_IGNORED_VEHICLE_TYPES:
                continue
    ...
    def potential_collision_manager(self, rx, ry, ryaw, waypoint, adjacent_check=False):
        """
        This module is in charge of warning in case of a collision.
        ...
        for vehicle in self.obstacle_vehicles:
            collision_free = self._potential_collision_check.collision_circle_check(
                rx, ry, ryaw, vehicle, self._ego_speed / 3.6, self._map,
                adjacent_check=adjacent_check)
    ...
    def run_step(
        ...
        if collision_detector_enabled:
            ...
            is_potential_hazard, potential_vehicle, distance = self.potential_collision_manager(
               rx, ry, ryaw, ego_vehicle_wp)
        ...
        if is_hazard:
            delta_v = max(1, (self._ego_speed - get_speed(obstacle_vehicle)) / 3.6)
            ttc = distance / delta_v if delta_v != 0 else distance / \
                                                        np.nextafter(0., 1.)
        ...
        if is_intersection:
            if is_hazard:
                obstacle_speed = get_speed(obstacle_vehicle)
            elif is_potential_hazard:
                obstacle_speed = get_speed(potential_vehicle)
            try:
                if obstacle_speed < 1:
                    target_speed, target_loc = self._local_planner.run_step(
                        rx, ry, rk, target_speed=self.max_speed
                        if not target_speed else target_speed)
                    return target_speed, target_loc
            except: 
                print("INTERSECTION BUT NO HAZARD ! ! !")


        if is_hazard:
            if distance < max(self.break_distance, 3):
                return 0, None
        if is_potential_hazard:
            if distance < 2:
                return 0, None
            elif distance < 4:
                print("Slowing down !")
                target_speed, target_loc = self._local_planner.run_step(
                    rx, ry, rk, target_speed=max(self._ego_speed - 5, 10)
                    if not target_speed else target_speed)
                return target_speed, target_loc
