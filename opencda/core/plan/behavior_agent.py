...
class BehaviorAgent(object):
    ...
    def __init__(self, vehicle, carla_map, config_yaml):
        ...
        self._collision_check = CollisionChecker(
            time_ahead=config_yaml['collision_time_ahead'], circle_radius=1)
        ...
        self.overtake_distance = config_yaml['overtake_distance']
    ...
    def stopping_vehicle_manager(self, rx, ry, ryaw, waypoint, adjacent_check=False):
        """
        Check if there is a stopped vehicle ahead.
        ...
        ryaw : list
            Yaw angles along the planned path.
        ego_waypoint : carla.Waypoint
            Current waypoint of the ego vehicle.
        stop_speed_threshold : float
            Speed threshold (m/s) to consider a vehicle as stopped.

        Returns
        -------
        is_stopped : bool
            True if a stopped vehicle is detected ahead.
        stopped_vehicle : carla.Vehicle or None
            The detected stopped vehicle, if any.
        distance_to_vehicle : float
            Distance to the stopped vehicle, if detected. Otherwise, a large number.
        """
        def dist(vehicle):
        ...
        for vehicle in self.obstacle_vehicles:
            # Check collision on the planned path
            collision_free = self._collision_check.collision_circle_check(
                rx, ry, ryaw, vehicle, self._ego_speed / 3.6, self._map
            )
            ...
            if not collision_free:
                ...
                distance = positive(dist(vehicle) - 3)  # Subtract approximate vehicle length
                
                # Check if the vehicle is stopped
                vehicle_speed = get_speed(vehicle)  # Use Carla's get_speed utility
                if vehicle_speed < 5 and distance < min_distance:
                    is_stopped = True
                    stopped_vehicle = vehicle
                    min_distance = distance
                    print(stopped_vehicle.type_id)

        return is_stopped, stopped_vehicle, min_distance
    ...
    def run_step(
        ...
        # 3-1. Stopping Vehicle check
        is_stopped = False
        is_stopped, stopping_vehicle, stopping_distance = self.stopping_vehicle_manager(
            rx, ry, ryaw, ego_vehicle_wp)
        ...
        # 6. overtake handeling
            ...
            if ego_lane_id == obstacle_lane_id:
                ...
                if distance < self.overtake_distance: ## execute overtake when the distance is smaller than the fixed distance
        ...
        # 6-1. If there is a stopping vehicle in front,
        if is_stopped:
            # if obstacle_vehicle != stopping_vehicle:
            stopping_lane_id = self._map.get_waypoint(stopping_vehicle.get_location()).lane_id
            ego_lane_id = self._map.get_waypoint(
                self._ego_pos.location).lane_id
            
            if ego_lane_id == stopping_lane_id:
                if stopping_distance < self.overtake_distance:
                    print("! ! ! There is a stopping vehicle, so overtaking ! ! !")
                    car_following_flag = self.overtake_management(obstacle_vehicle)
